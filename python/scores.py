...
class scores():
    def __init__(self, seg1, seg2, eps_theta, image, grayscale, ratio):
        self.seg1 = seg1
        self.seg2 = seg2
        self.eps_theta = eps_theta
        self.image = image
        self.th_grayscale = grayscale
        self.th_ratio = ratio
    ...
    def proximity(self):
        ...
        r_seg1 = self.seg1[0, 0]
        r_seg2 = self.seg2[0, 0]
    
        t_seg1 = self.seg1[1, 0]
        t_seg2 = self.seg2[1, 0]
    
        y1_seg1 = self.seg1[2, 0]
        y1_seg2 = self.seg2[2, 0]
        x1_seg1 = (r_seg1 - y1_seg1 * math.sin(t_seg1)) / math.cos(t_seg1)
        x1_seg2 = (r_seg2 - y1_seg2 * math.sin(t_seg2)) / math.cos(t_seg2)
    
        y2_seg1 = self.seg1[3, 0]
        y2_seg2 = self.seg2[3, 0]
        x2_seg1 = (r_seg1 - y2_seg1 * math.sin(t_seg1)) / math.cos(t_seg1)
        x2_seg2 = (r_seg2 - y2_seg2 * math.sin(t_seg2)) / math.cos(t_seg2)
    
        p_seg1 = self.seg1[4, 0]
        p_seg2 = self.seg2[4, 0]
    
        y0_seg1 = self.seg1[5, 0]
        y0_seg2 = self.seg2[5, 0]
        ...
        if score == 1:
            if abs(t_seg1 - t_seg2) > self.eps_theta:
                score = 0
        # rule3:
        ...
        # another brute method to calculate bright area/total area
        """
        image = self.image
        if score == 1:
            x1_seg1 = int(x1_seg1)
    ...
    def check_color(self):
        ...
        points = np.array([[[x2_seg1, 300-y2_seg1],[x2_seg2, 300-y2_seg2],[x1_seg2, 300-y1_seg2],[x1_seg1, 300-y1_seg1]]], dtype=np.int32)
